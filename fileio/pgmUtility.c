...
int ** pgmRead( char **header, int *numRows, int *numCols, FILE *in )
{
    ...
    {
    	fgets(curr, maxSizeHeadRow, in);
    	if ((*(header + i) = (char *)malloc(strlen(curr) * sizeof(char))) == NULL)
        {
	...
	for (i = 0; i < *numRows; i++)
	{
        ...
        {
            printf("Error allocating space for **pixels\n");
            return NULL;
        }
	}
	for (i = 0; i < *numRows; i++)
	{
		for (j = 0; j < *numCols; j++)
		{
            ...
            {
                printf("Error reading pixel information.  Make sure input file is valid.\n");
                return NULL;
            }
		}
	}
